...
def load_ARC_data(Path_csv_file, lengt, day, skip):
    ...
    file_ARC_data['period'] = pd.to_datetime(
        file_ARC_data['period'], format='%Y/%m/%d %H:%M')  # column period in datetime format
...
def main_program_147(dataset_id='ods147'):
    ...
    final.to_csv(
        f'finals/{day_run2}_{str(qua_end)}NQH_imb_price_prediction.csv', index=False)
...
# def generate_figs():
    ...
    # fig.show()
    # fig.write_html(f'finals/{day_run2}_NQHimb_price_prediction.html')
    ...
    # fig1.write_html(f'finals/{day_run2}_NQHERROR_imb_price_prediction.html')
